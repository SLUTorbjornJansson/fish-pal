...
$set fileName %resDir%\simulation\outputForPresentation_%runtype%%scenario_path_underScores%%ResId%
...
EXECUTE_UNLOAD "%fileName%.gdx" outputEconomics effortAnnual catchInfo quotaUptake;
...
execute "GDXXRW i=%fileName%.gdx o=%fileName%.xlsx par=outputEconomics rng=outputEconomics!A1 cdim=2 rdim=1" ;
execute "GDXXRW i=%fileName%.gdx o=%fileName%.xlsx par=effortAnnual rng=effortAnnual!A1 cdim=0 rdim=1" ;
execute "GDXXRW i=%fileName%.gdx o=%fileName%.xlsx par=catchInfo rng=catchInfo!A1 cdim=3 rdim=1" ;
execute "GDXXRW i=%fileName%.gdx o=%fileName%.xlsx par=quotaUptake rng=quotaUptake!A1 " ;
...
EXECUTE_UNLOAD "%fileName%.gdx" p_margValueIndQuota ;
