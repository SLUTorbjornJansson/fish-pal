...
e_catch(f,s) $ fishery_species(f,s) ..
    v_catch(f,s) =E= pv_delta(f,s)*v_effortAnnual(f)**p_beta(f,s);
*    v_catch(f,s) =E= p_catchDistKilograms(f,s)/1000*pv_delta(f)*v_effortAnnual(f)**p_beta(f,s);
...
*    + SUM(s $ fishery_species(f,s), v_lambdaCatch(f,s)*pv_delta(f,s)*p_beta(f,s)*v_effortAnnual(f)**(p_beta(f,s)-1))
