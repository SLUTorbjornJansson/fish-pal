...
e_focEffortAnnual(f) $ [v_effortAnnual.LO(f) LT v_effortAnnual.UP(f)] .. // Don't build this equation if effort is fixed
    ...
    + SUM(s $ fishery_species(f,s), v_lambdaCatch(f,s)*v_CPUE(f,s))
...
*###############################################################################
*            SET BOUNDS FOR VARIABLES COMMON TO ESTIMATION AND SIMULATION
*###############################################################################

* (nothing to do here...)

*#############################################################
*            CALIBRATE MODEL TO BASELINE SITUATION
*#############################################################
