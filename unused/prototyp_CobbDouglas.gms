...
*    p_catchSlope(fishery)       "Slope term of marginal catch function"
    p_beta(fishery,species)     "Elasticity of catch to annual effort, or b1 if catch=a*E^b1*S^b2"
...
*##############################################################################
*  DEFINE DEFAULT BEHAVIOURAL PARAMETERS
*##############################################################################

*   Default assumption is that all fisheries are close to constant returns to scale
*   (Frost et al. 2009, EIAA-modellen)
p_beta(f,s) $ fishery_species(f,s) = 0.95;

*   For certain fisheries, studies have revealed decreasing returns
* <to do>
...
*   Primal model equations
    ...
    e_delta(fishery,species)        "Scaling factor for C-D catch function defined from the fixed catch distribution and a single variable per fishery"
...
*   Primal model variables
    ...
    v_catchScale(fishery)           "Scaling factor for C-D catch function uniformly applied to all species of each fishery"
...
e_catch(f,s) $ fishery_species(f,s) ..
    v_catch(f,s) =E= p_catchDistKilograms(f,s)/1000*v_catchScale(f)*v_effortAnnual(f)**p_beta(f,s);
...
*   Some auxiliary variables that do not influence the solution but are
*   convenient for understanding the model. Need no FOC in the estimation.

*   AVERAGE catch per unit of effort (v_CPUE, in tons per day) matching total catch
e_CPUE(f,s) $ fishery_species(f,s) ..

     v_catch(f,s) =E= v_CPUE(f,s)*v_effortAnnual(f) ;


e_delta(f,s) $ fishery_species(f,s) ..
    pv_delta(f,s) =E= p_catchDistKilograms(f,s)/1000*v_catchScale(f);
...
*   Variable cost sum as deviation from distribution mean (observed value)
     SUM(f, SQR(pv_varCostSum(f)-p_varCostSumDist(f,"mean"))/p_varCostSumDist(f,"var"))

*   Annual effort should be "close" to observed effort
    +SUM(f, SQR(v_effortAnnual(f)-p_effort2009(f)))

*   Annual catch should be "close" to observed catch
    +SUM((f,s), SQR((v_catch(f,s)-p_effort2009(f)*p_catchDistKilograms(f,s))/1000))
...
e_focEffortAnnual(f) $ [v_effortAnnual.LO(f) LT v_effortAnnual.UP(f)] .. // Don't build this equation if effort is fixed
    -pv_varCostSum(f)
*    + SUM(s $ fishery_species(f,s), v_lambdaCatch(f,s)*v_catch(f,s)/v_effortAnnual(f)*p_beta(f,s))
    + SUM(s $ fishery_species(f,s), v_lambdaCatch(f,s)*p_catchDistKilograms(f,s)/1000*v_catchScale(f)*p_beta(f,s)*v_effortAnnual(f)**(p_beta(f,s)-1))
...
e_csEffNonNeg(f) ..
*   Complementary slackness of ea > ea.LO, i.e. lambda*(ea-ea.LO) = 0. Before, ea.LO was zero, hence the name "NonNeg"
    [v_effortAnnual(f)-v_effortAnnual.LO(f)]*v_lambdaEffNonNeg(f)
    =L= p_mu;
...
e_csEffNonNegBinSlack(f) ..
    [v_effortAnnual(f)-v_effortAnnual.LO(f)] =L= (1-v_effNonNegIsBinding(f))*10E6;
...
MODEL m_fishSim "Primal simulation model with profit maximization"
    /e_objFunc,e_catch,e_effRestrSeg,e_effRestrFishery,e_catchQuota,e_CPUE/;
...
*   Primal model constraints must be satisfied
    e_catch,e_effRestrSeg,e_effRestrFishery,e_catchQuota,e_delta,

*   Reporting equations for primal model (not constraints, just free variables)
    e_CPUE,
...
*   Primal model constraints must be satisfied
    e_CPUE,e_catch,e_effRestrSeg,e_effRestrFishery,e_catchQuota,e_delta,
...
v_effortAnnual.L(f) = p_effort2009(f);

* Starting point for delta derived from observed effort and catch as delta=catch*ea**(1-p_beta)
pv_delta.L(f,s) $ fishery_species(f,s) = p_catchDistKilograms(f,s)/1000*p_effort2009(f)**(1-p_beta(f,s));

v_catchScale.L(f) = 1;

* Preparatory computations: average and marginal profit per unit of effort for each fishery

p_profit2009(f,"aveReve") = SUM(s, p_prices(f,s)*p_catchDistKilograms(f,s)/1000);
...
*       ... and variance (inverse weight in estimation) trivially "1"
p_varCostSumDist(fishery,"var")  = 1;
...
*m_estimateFish.ITERLIM = 1;
*p_mu = 0;
*SOLVE m_estimateFish USING NLP MINIMIZING v_estimationMetric;   //test that model is feasible in starting point, and equations are correctly specified
...
* Stäng av rapportering av modellösningarna, eftersom vi ska göra många iterationer
...
* Gör en sista lösning utan approximation (mu=0), och visa resultatet
* Detta är startpunkten till BARON, MINLP-solvern
*
m_estimateFish.SOLPRINT = 1;   // Slå på full rapportering av lösning
p_mu = 0;                      // Sätt my till noll så att det inte är en approximation, utan exakta komplementaritetsvillkor
SOLVE m_estimateFish USING NLP MINIMIZING v_estimationMetric; // Lös modellen...
...
* Kolla vad som händer om vi löser den primala modellen nu.
